...
{
    internal static class DTOMapperConfigurationExtensions
    {
        public static void CreateAutoAttributeMaps(this IMapperConfigurationExpression configuration, params Type[] types)
        {
            foreach (var type in types)
            {

                foreach (var autoMapAttribute in type.GetTypeInfo().GetCustomAttributes<DTOMapAttributeBase>(true))
                {
                    if (autoMapAttribute.GetType() == typeof(DTOMapAttribute))
                    {
                        var TargetTypes = autoMapAttribute.TargetTypes;
                    ...
                    else if (autoMapAttribute.GetType() == typeof(DTOMapFromAttribute))
                    {
                        var map = (DTOMapFromAttribute)autoMapAttribute;
                        var TargetTypes = map.TargetTypes;
                        var MemberList = (MemberList)(int)map.MemberList;
                        if (TargetTypes == null || TargetTypes.Length == 0)
                        {
                            return;
                        }
                    ...
                    else if (autoMapAttribute.GetType() == typeof(DTOMapToAttribute))
                    {
                        var map = (DTOMapToAttribute)autoMapAttribute;
                        var TargetTypes = map.TargetTypes;
                        var MemberList = (MemberList)(int)map.MemberList;
                        if (TargetTypes == null || TargetTypes.Length == 0)
                        {
        ...
        public static void CreateAutoAttributeMaps(this IMapperConfigurationExpression configuration, Type type, Type[] targetTypes, AutoMapper.MemberList memberList)
        {

            //Get all the properties in the source that have the AutoMapKeyAttribute
            var sourceKeysPropertyInfo = type.GetProperties()
                                             .Where(w => w.GetCustomAttribute<DTOMapKeyAttribute>() != null)
                                             .Select(s => s).ToList();
